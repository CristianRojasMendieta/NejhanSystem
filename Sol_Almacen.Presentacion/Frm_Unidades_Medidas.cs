...
namespace Sol_Almacen.Presentacion
{
    public partial class Frm_Unidades_Medidas : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgv_Listado.Columns[1].HeaderText = "CÓDIGO UM";
            Dgv_Listado.Columns[2].Width = 200;
            Dgv_Listado.Columns[2].HeaderText = "ABREVIATURA";
            Dgv_Listado.Columns[3].Width = 250;
            Dgv_Listado.Columns[3].HeaderText = "UNIDAD MEDIDA";
        }
        ...
        private void Mostrar(string Ctexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Unidades_Medidas.Mostrar_um(Ctexto);
                this.Formato();
        ...
        private void Selecciona_dgv()
        {
            ...
            else
            {
                ...
                Txt_abreviatura_um.Text = Convert.ToString(Dgv_Listado.CurrentRow.Cells["abreviatura_um"].Value);
        ...
        private void Estado_Limpiar(bool Lestado)
        {
            ...
            Txt_buscar.Clear();
            Txt_codigo_um.Text = "0";
            Txt_abreviatura_um.Text = "";
            Txt_descripcion_um.Text = "";
            Txt_abreviatura_um.ReadOnly = !Lestado;
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txt_codigo_um.Text == string.Empty && Txt_descripcion_um.Text == string.Empty)
                {
                    MessageBox.Show("Falta ingresar datos requeridos (*)", "Aviso del Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    E_Unidades_Medidas Eum = new E_Unidades_Medidas();
                    string Cduplica, Rpta = "";
                    Eum.Codigo_um = Convert.ToInt32(Txt_codigo_um.Text);
                    Eum.Abreviatura_um = Txt_abreviatura_um.Text.Trim();
                    Eum.Descripcion_um = Txt_descripcion_um.Text.Trim();
                    Eum.Estado = true;
                    // Verificamos si la información que se intenta guardar ya existe en la tabla
                    Cduplica = N_Unidades_Medidas.Verifica_duplicado_um(this.EstadoGuarda, Eum.Codigo_um, Eum.Descripcion_um);
                    if (Cduplica == "")
                    {
                        Rpta = N_Unidades_Medidas.Guardar_um(this.EstadoGuarda, Eum);
                        if (Rpta.Equals("OK"))
                        {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in Dgv_Listado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = N_Unidades_Medidas.Eliminar_um(Codigo);
                            if (Rpta.Equals("OK"))
                            {

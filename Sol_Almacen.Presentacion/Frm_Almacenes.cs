...
namespace Sol_Almacen.Presentacion
{
    public partial class Frm_Almacenes : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgv_Listado.Columns[1].HeaderText = "CÓDIGO AL";
            Dgv_Listado.Columns[2].Width = 300;
            Dgv_Listado.Columns[2].HeaderText = "ALMACEN";
            Dgv_Listado.Columns[3].Width = 320;
            Dgv_Listado.Columns[3].HeaderText = "EMPRESA";
        ...
        private void Mostrar(string Ctexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Almacenes.Mostrar_al(Ctexto);
                this.Formato();
        ...
        {
            if (String.IsNullOrEmpty(Convert.ToString(Dgv_Listado.CurrentRow.Cells["codigo_al"].Value)))
            {
        ...
        private void Estado_Limpiar(bool Lestado)
        {
            ...
            Txt_buscar.Clear();
            Txt_codigo_al.Text = "0";           
            Txt_descripcion_al.Text = "";            
            Txt_descripcion_al.ReadOnly = !Lestado;
        }
        ...
        {
            Dgv_Listado_em.Columns[0].Width = 250;
            Dgv_Listado_em.Columns[0].HeaderText = "EMPRESA";
            Dgv_Listado_em.Columns[1].Visible = false;
        }

        private void Listar_em(string Ctexto)
        {
            try
            {
                Dgv_Listado_em.DataSource = N_Almacenes.Listar_em(Ctexto);
                this.Formato_em();
        ...
        private void Selecciona_dgv_em()
        {
            Txt_codigo_em.Text = Convert.ToString(Dgv_Listado_em.CurrentRow.Cells["codigo_em"].Value);
            Txt_descripcion_em.Text = Convert.ToString(Dgv_Listado_em.CurrentRow.Cells["razonsocial_em"].Value);
        }
        #endregion
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txt_codigo_al.Text == string.Empty && Txt_codigo_em.Text == string.Empty && Txt_descripcion_al.Text == string.Empty)
                {
                    MessageBox.Show("Falta ingresar datos requeridos (*)", "Aviso del Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    E_Almacenes Eal = new E_Almacenes();
                    string Cduplica, Rpta = "";
                    Eal.Codigo_al = Convert.ToInt32(Txt_codigo_al.Text);
                    Eal.Codigo_em = Convert.ToInt32(Txt_codigo_em.Text);
                    Eal.Descripcion_al = Txt_descripcion_al.Text.Trim();
                    Eal.Estado = true;
                    // Verificamos si la información que se intenta guardar ya existe en la tabla
                    Cduplica = N_Almacenes.Verifica_duplicado_al(this.EstadoGuarda, Eal.Codigo_al, Eal.Descripcion_al);
                    if (Cduplica == "")
                    {
                        Rpta = N_Almacenes.Guardar_al(this.EstadoGuarda, Eal);
                        if (Rpta.Equals("OK"))
                        {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in Dgv_Listado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = N_Almacenes.Eliminar_al(Codigo);
                            if (Rpta.Equals("OK"))
                            {
        ...
        {
            Pnl_listado_em.Visible = true;
            Txt_buscar_em.Select();
            this.Listar_em(Txt_buscar_em.Text);
        }

        private void Btn_retornar_gr_Click(object sender, EventArgs e)
        {
            Pnl_listado_em.Visible = false;
        }

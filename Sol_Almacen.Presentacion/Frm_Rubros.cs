...
namespace Sol_Almacen.Presentacion
{
    public partial class Frm_Rubros : Form
    {
        ...
        private void Formato()
        {
            ...
            Dgv_Listado.Columns[2].HeaderText = "RUBRO";
        ...
        private void Mostrar(string Ctexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Rubros.Mostrar_ru(Ctexto);
                this.Formato();
        ...
        {
            if (String.IsNullOrEmpty(Convert.ToString(Dgv_Listado.CurrentRow.Cells["codigo_ru"].Value)))
            {
        ...
        private void Estado_Limpiar(bool Lestado)
        {
            ...
            Txt_buscar.Clear();
            Txt_codigo_ru.Text = "0";           
            Txt_descripcion_ru.Text = "";            
            Txt_descripcion_ru.ReadOnly = !Lestado;
        }
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txt_codigo_ru.Text == string.Empty && Txt_descripcion_ru.Text == string.Empty)
                {
                    MessageBox.Show("Falta ingresar datos requeridos (*)", "Aviso del Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    E_Rubros Eru = new E_Rubros();
                    string Cduplica, Rpta = "";
                    Eru.Codigo_ru = Convert.ToInt32(Txt_codigo_ru.Text);                    
                    Eru.Descripcion_ru = Txt_descripcion_ru.Text.Trim();
                    Eru.Estado = true;
                    // Verificamos si la información que se intenta guardar ya existe en la tabla
                    Cduplica = N_Rubros.Verifica_duplicado_ru(this.EstadoGuarda, Eru.Codigo_ru, Eru.Descripcion_ru);
                    if (Cduplica == "")
                    {
                        Rpta = N_Rubros.Guardar_ru(this.EstadoGuarda, Eru);
                        if (Rpta.Equals("OK"))
                        {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in Dgv_Listado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = N_Rubros.Eliminar_ru(Codigo);
                            if (Rpta.Equals("OK"))
                            {
